...
//-----------------------------------------------------------------------------------------------------------------------------------------------------
// 例8：Hard 地牢游戏 选自LeetCoede 174. Dungeon Game 
// 已知一个二维数组，左上角代表骑士的位置，右下角代表公主的位置，二维数组中存储整数，正数可以给骑士增加生命值，负数会减少骑士的生命值，
// 问骑士初始时至少是多少生命值，才可保证骑士在行走的过程中至少保持生命值为1。(骑士只能向下或向右行走)

// 思考：
// 直接思考动态规划:
// 我们应该从左上向右下递推，还是从右下向左上递推?若用一个二维数组代表每个格子的状态，dp[i][j]具体代表什么?
// 若左上向右下递推:
// 可以递推出在每个格子最多能获得多少血量，即dp[i][j]代表骑士在位置(i, j)可以积累的最大血量，能否转换成初始时至少是多少血量?
// 若右下向左上递推:
// 应如何设计动态规划状态，二维数组dp[i][j]代表了什么?

// 算法思路：
// 从左上向右下递推:
// 没办法将"每个格子最多能获得多少血量"，转换成"初始时至少是多少血量"。

// 从右下向左上递推:
// dp[i][j]即代表若要达到右下角，至少有多少血量，能在行走的过程中至少保持生命值为1。

// 例如:
// 若代表地牢的二维数组为1*1的:dp[0][0]=max(1，1-dungeon[0][0])
